...
import { SavedPhotoItem, COLOR_PRESETS, CURTAIN_STYLES, HASHTAG_PRESETS, HOUSE_TYPES, DEVELOPERS, EmployeeUser, MasterDataConfigs } from '../types';
...
interface PortfolioPageProps {
  ...
  onOpenLightbox: (photo: SavedPhotoItem, list?: SavedPhotoItem[]) => void;
  title?: string;
  isFavoriteOnly?: boolean;
  activeUser?: EmployeeUser;
  onEditPhoto?: (photo: SavedPhotoItem) => void;
  onSharePhoto?: (photo: SavedPhotoItem) => void;
  onToggleHidePhoto?: (id: string) => void;
  onToggleHideFolder?: (villageName: string, hide: boolean) => void;
  onUpdateFolderCovers?: (covers: { [village: string]: string }) => Promise<void>;
}
...
export default function PortfolioPage({
  ...
  // Derive unique available filter options dynamically from uploaded photos
  const availableHouseTypes = useMemo(() => {
    const types = photos
      .map(p => p.houseType?.trim())
      .filter(Boolean);
    return Array.from(new Set(types)).sort((a, b) => a.localeCompare(b, 'th'));
  }, [photos]);

  const availableCurtainStyles = useMemo(() => {
    const stylesSet = new Set<string>();
    photos.forEach(p => {
      if (p.curtainStyle) {
        p.curtainStyle.split(',').forEach(s => {
    ...
    return Array.from(stylesSet).sort((a, b) => a.localeCompare(b, 'th'));
  }, [photos]);
  ...
  const availableDevelopers = useMemo(() => {
    const devs = photos
      ...
      .filter(Boolean);
    return Array.from(new Set(devs)).sort((a, b) => a.localeCompare(b, 'th'));
  }, [photos]);
  ...
  const availableHashtags = useMemo(() => {
    ...
    return Array.from(tagsSet).sort((a, b) => a.localeCompare(b, 'th'));
  }, [photos]);

  const availableEmployees = useMemo(() => {
    const emps = photos
      .map(p => p.employee?.trim())
      .filter(Boolean);
    return Array.from(new Set(emps)).sort((a, b) => a.localeCompare(b, 'th'));
  }, [photos]);
  ...
  const filteredPhotos = useMemo(() => {
    ...
    // Filter out hidden photos if user is not admin
    ...
    // Filter by house type category (robust substring check)
    if (selectedHouseType !== 'all') {
      result = result.filter(p => {
        if (!p.houseType) return false;
        const hType = p.houseType.toLowerCase();
        const selType = selectedHouseType.toLowerCase();
        return hType === selType || hType.includes(selType) || selType.includes(hType);
      });
    }

    // Filter by curtain style (robust comma-separated and substring check)
    if (selectedStyle !== 'all') {
      result = result.filter(p => {
        if (!p.curtainStyle) return false;
        const styles = p.curtainStyle.split(',').map(s => s.trim().toLowerCase());
        const target = selectedStyle.toLowerCase();
        return styles.some(s => s === target || s.includes(target) || target.includes(s));
      });
    }

    // Filter by developer (robust substring check)
    if (selectedDeveloper !== 'all') {
      result = result.filter(p => {
        if (!p.developer) return false;
        const dev = p.developer.toLowerCase();
        const selDev = selectedDeveloper.toLowerCase();
        return dev === selDev || dev.includes(selDev) || selDev.includes(dev);
      });
    }

    // Filter by hashtags (robust tag-matching)
    if (selectedHashtag !== 'all') {
      result = result.filter(p => {
        if (!p.hashtags) return false;
        const target = selectedHashtag.toLowerCase();
        return p.hashtags.some(tag => {
    ...
    // Filter by employee (robust name check)
    if (selectedEmployee !== 'all') {
      result = result.filter(p => {
        if (!p.employee) return false;
        const emp = p.employee.toLowerCase();
        const selEmp = selectedEmployee.toLowerCase();
        return emp === selEmp || emp.includes(selEmp) || selEmp.includes(emp);
      });
    }
    ...
    return result;
  }, [photos, isFavoriteOnly, searchQuery, selectedHouseType, selectedStyle, selectedDeveloper, selectedHashtag, selectedEmployee]);
  ...
  useEffect(() => {
    if (configs?.folderCovers) {
      setFolderCovers(configs.folderCovers);
    } else {
      // Fallback to localStorage
      try {
        const saved = localStorage.getItem('village_folder_covers');
        if (saved) setFolderCovers(JSON.parse(saved));
      } catch (err) {
        console.error(err);
      }
    }
  ...
  const handleSetFolderCover = async (village: string, photoId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (activeUser?.role !== 'admin') {
      alert('เฉพาะผู้ดูแลระบบ (Admin) เท่านั้นที่สามารถตั้งรูปภาพหน้าปกโครงการได้');
      return;
    }
    ...
    if (onUpdateFolderCovers) {
      await onUpdateFolderCovers(updated);
    }
  };
        ...
        <div className="bg-white/50 backdrop-blur-xl border border-white/40 rounded-[32px] p-5 shadow-xl space-y-4">
          <div className="flex gap-2 items-center">
            ...
            >
              ...
              {(selectedHouseType !== 'all' || selectedStyle !== 'all' || selectedDeveloper !== 'all' || selectedHashtag !== 'all' || selectedEmployee !== 'all') && (
            ...
            <div className="pt-3 border-t border-slate-200/50 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 select-none animate-fadeIn">
              ...
              {availableHouseTypes.length > 0 && (
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">ประเภทที่อยู่อาศัย</label>
                  <div className="relative">
                    ...
                    >
                      <option value="all">ทั้งหมด (ทุกประเภทบ้าน)</option>
                      {availableHouseTypes.map(type => (
              ...
              {availableCurtainStyles.length > 0 && (
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">รูปแบบผ้าม่าน</label>
                  <div className="relative">
                    <select
                      value={selectedStyle}
                      ...
                      className="w-full pl-3 pr-8 py-2 rounded-xl border border-slate-200 bg-white/50 text-xs focus:outline-none cursor-pointer"
                    >
                      <option value="all">ทั้งหมด (ทุกรูปแบบทรง)</option>
                      {availableCurtainStyles.map(style => (
              ...
              {availableDevelopers.length > 0 && (
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Developer</label>
                  <div className="relative">
                    <select
                      ...
                      className="w-full pl-3 pr-8 py-2 rounded-xl border border-slate-200 bg-white/50 text-xs focus:outline-none cursor-pointer"
                    >
                      <option value="all">ทั้งหมด (ทุกโครงการผู้พัฒนา)</option>
              ...
              {availableHashtags.length > 0 && (
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">#Hashtage</label>
                  <div className="relative">
                    <select
                      ...
                      className="w-full pl-3 pr-8 py-2 rounded-xl border border-slate-200 bg-white/50 text-xs focus:outline-none cursor-pointer"
                    >
                      <option value="all">ทั้งหมด (ทุก Hashtag)</option>
              ...
              {availableEmployees.length > 0 && (
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">พนักงาน</label>
                  <div className="relative">
                    <select
                      ...
                      className="w-full pl-3 pr-8 py-2 rounded-xl border border-slate-200 bg-white/50 text-xs focus:outline-none cursor-pointer"
                    >
                      <option value="all">ทั้งหมด (ทุกรายชื่อพนักงาน)</option>
            ...
            {activeFolder.items.map((photo, index) => {
              ...
              return (
                <PhotoCard
                  ...
                  onOpenLightbox={(p) => onOpenLightbox(p, activeFolder.items)}
                  ...
                  >
                    ...
                    <div className={`relative aspect-[4/3] ${isS ? 'mb-2.5' : 'mb-4'}`}>
                      ...
                      <div className="absolute inset-0 rounded-2xl overflow-hidden bg-slate-100 flex items-center justify-center shadow-md border border-slate-200/20 z-10">
                        ...
                        {/* Admin-only Hide Folder Toggle */}
                        {activeUser?.role === 'admin' && onToggleHideFolder && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              const isCurrentlyHidden = folder.items.every(p => p.hidden);
                              onToggleHideFolder(folder.name, !isCurrentlyHidden);
                            }}
                            className={`absolute z-20 rounded-full bg-black/60 flex items-center justify-center backdrop-blur shadow transition-all cursor-pointer hover:scale-105 hover:bg-red-600
                              ${isS ? 'top-2 left-2 w-6 h-6' : 'top-3 left-3 w-8 h-8'}
                              ${folder.items.every(p => p.hidden) ? 'text-red-500 bg-white/90 scale-105' : 'text-white'}
                            `}
                            title={folder.items.every(p => p.hidden) ? "แสดงโฟลเดอร์" : "ซ่อนโฟลเดอร์"}
                          >
                            {folder.items.every(p => p.hidden) ? <EyeOff size={isS ? 10 : 13} /> : <Eye size={isS ? 10 : 13} />}
                          </button>
                        )}

                        {/* Folder Hidden Badge */}
                        {folder.items.every(p => p.hidden) && (
                          <div className={`absolute z-20 bg-red-600 text-white font-extrabold rounded-lg shadow-md select-none
                            ${isS ? 'bottom-2 left-2 px-1.5 py-0.5 text-[8px]' : 'bottom-3 left-3 px-2.5 py-1 text-[9px]'}`}
                          >
                            👁 โฟลเดอร์ถูกซ่อนอยู่
                          </div>
                        )}
                    ...
                    <div className={`mt-3 border-t border-slate-200/50 flex justify-between items-center text-slate-500 font-semibold ${isS ? 'pt-2.5 text-[10px]' : 'pt-3.5 text-xs'}`}>
                      <div className="flex flex-wrap gap-1 max-w-[70%]">
                        {samplePhoto?.curtainStyle ? (
                          samplePhoto.curtainStyle.split(',').map(s => s.trim()).map((style, idx) => {
                            const name = style.split(' ')[0];
                            return name ? (
                              <span 
                                key={idx} 
                                className={`bg-slate-100 text-slate-600 rounded-lg font-extrabold ${
                                  isS ? 'text-[8px] px-1 py-0.5' : 'text-[9px] px-1.5 py-0.5'
                                }`}
                              >
                                {name}
                              </span>
                          ...
                          <span className={`bg-slate-100 text-slate-600 rounded-lg font-extrabold ${isS ? 'text-[8px] px-1 py-0.5' : 'text-[9px] px-1.5 py-0.5'}`}>
                            ผ้าม่าน
                          </span>
                        )}
                      </div>
                      <button
                        ...
                        className={`rounded-lg hover:bg-indigo-50 hover:text-indigo-600 transition-all flex items-center gap-1 flex-shrink-0 ${isS ? 'p-1 text-[9px]' : 'p-1.5 text-[10px]'}`}
            ...
            <div className={`grid ${gridClass}`}>
              {sortedPhotos.map((photo) => (
                <PhotoCard
                  ...
                  onToggleLike={onToggleLike}
                  onDeletePhoto={onDeletePhoto}
                  onOpenLightbox={(p) => onOpenLightbox(p, sortedPhotos)}
                  triggerImageDownload={triggerImageDownload}
                  activeUser={activeUser}
                  onEditPhoto={onEditPhoto}
                  onSharePhoto={onSharePhoto}
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-slate-100 flex-shrink-0">
        ...
        {/* Hidden Photo Badge Overlay */}
        {photo.hidden && (
          <div className={`absolute top-2.5 z-10 px-2.5 py-1 rounded-lg bg-red-600 text-white text-[9px] font-extrabold shadow-md flex items-center gap-1 select-none ${isInsideFolder ? 'left-24' : 'left-2.5'}`}>
            👁 ภาพถูกซ่อนอยู่
          </div>
        )}
        ...
        {(activeUser?.role === 'admin' || 
          ...
          <div className="absolute bottom-2.5 right-2.5 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity z-10 select-none">
            {/* Toggle Hide Button (Admin only) */}
            {activeUser?.role === 'admin' && onToggleHidePhoto && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onToggleHidePhoto(photo.id);
                }}
                className={`w-7 h-7 rounded-full bg-black/60 flex items-center justify-center backdrop-blur shadow transition-all cursor-pointer hover:scale-105
                  ${photo.hidden ? 'text-red-500 bg-white/90 scale-105' : 'text-white hover:text-red-400'}
                `}
                title={photo.hidden ? "ยกเลิกการซ่อนรูปภาพ" : "ซ่อนรูปภาพ"}
              >
                {photo.hidden ? <EyeOff size={10} /> : <Eye size={10} />}
              </button>
            )}
        ...
        <div className="p-2.5 flex-1 flex flex-col justify-between space-y-1.5">
          ...
          <div className="pt-1.5 border-t border-slate-200/50 flex flex-col gap-1 text-[9px] text-slate-500">
            <div className="flex flex-wrap gap-1 items-center w-full">
              {photo.curtainStyle ? (
                photo.curtainStyle.split(',').map(s => s.trim()).map((style, idx) => {
                  const name = style.split(' ')[0];
                  return name ? (
                    <span key={idx} className="font-semibold text-slate-700 bg-slate-100 px-1.5 py-0.5 rounded-md text-[9px]">
                      {name}
                    </span>
                  ) : null;
                })
